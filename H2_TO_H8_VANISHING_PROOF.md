...
three-slot full-ICO strategy. After the $SU(2)\times SU(2)$ and $S_3$ twirls,
...
\qquad
\Phi_U:=J_U^T
=\lvert U^*\rangle\!\rangle\langle\!\langle U^*\rvert.
$$
...
$$
\begin{aligned}
&\mathbb E_U\frac14\operatorname{Tr}\!\left[
\left(\bar C*\big((1-\gamma)J_U+\tfrac\gamma2I_4\big)^{\otimes3}\right)J_U
\right]\\
&\qquad=\operatorname{Tr}(\Omega_\gamma\bar C),
\end{aligned}
\tag{2}
$$
...
\Omega_\gamma
:=
\frac14\mathbb E_U\!\left[
\left((1-\gamma)\Phi_U+\frac\gamma2I_4\right)^{\otimes3}_{I^3O^3}
\otimes(J_U)_{PF}
\right].
\tag{3}
$$

Thus $\operatorname{Tr}(\Omega_\gamma\bar C)$ is exactly the Haar-averaged
...
\begin{aligned}
G^\dagger\bar C G
=\operatorname{diag}\big(&H_0\otimes I_1,H_1\otimes I_3,
...
$$
{}_X C:=\frac{I_X}{d_X}\otimes\operatorname{Tr}_X C.
$$

The deterministic full-ICO constraints and the objective in Eq. (2) are
preserved by the $SU(2)\times SU(2)$ covariance twirl and by simultaneous
...
$$
\begin{aligned}
\underset{\bar C}{\operatorname{maximize}}\quad
&\operatorname{Tr}(\Omega_\gamma\bar C)\\[1mm]
...
&\emptyset\ne S\subseteq\{1,2,3\},\\
{}_{I^3O^3F}\bar C={}_{PI^3O^3F}\bar C,\\
\operatorname{Tr}\bar C=16,\\
[\bar C,U_{V,W}]=0,
&V,W\in SU(2),\\
...
$$
U_{V,W}
=V_P\otimes V_{I^3}^{*\otimes3}
\otimes W_{O^3}^{*\otimes3}\otimes W_F.
$$
...
$$
\begin{aligned}
&\left((1-\gamma)\Phi_U+\frac\gamma2I_4\right)^{\otimes3}\\
={}&\left(\frac\gamma2\right)^3 I_4\otimes I_4\otimes I_4\\
&+(1-\gamma)\left(\frac\gamma2\right)^2
\big(\Phi_U\otimes I_4\otimes I_4
+I_4\otimes\Phi_U\otimes I_4
+I_4\otimes I_4\otimes\Phi_U\big)\\
&+(1-\gamma)^2\left(\frac\gamma2\right)
...
$$
\begin{aligned}
\Omega^{(0)}&=\frac14\mathbb E_U[I_4^{\otimes3}\otimes J_U],\\
\Omega^{(1)}&=\frac1{12}\mathbb E_U[(\Phi_U\otimes I_4\otimes I_4
+I_4\otimes\Phi_U\otimes I_4+I_4\otimes I_4\otimes\Phi_U)\otimes J_U],\\
\Omega^{(2)}&=\frac1{12}\mathbb E_U[(\Phi_U\otimes\Phi_U\otimes I_4
+\Phi_U\otimes I_4\otimes\Phi_U+I_4\otimes\Phi_U\otimes\Phi_U)\otimes J_U],\\
\Omega^{(3)}&=\frac14\mathbb E_U[\Phi_U^{\otimes3}\otimes J_U].
\end{aligned}
\tag{6}
$$
...
\begin{aligned}
\Omega_\gamma={}&
\left(\frac\gamma2\right)^3\Omega^{(0)}
+3(1-\gamma)\left(\frac\gamma2\right)^2\Omega^{(1)}\\
&+3(1-\gamma)^2\left(\frac\gamma2\right)\Omega^{(2)}
+(1-\gamma)^3\Omega^{(3)}.
\end{aligned}
\tag{7}
$$
...
$$
\operatorname{Tr}(\Omega^{(m)}\bar C)
=\sum_{k=0}^{8}\operatorname{Tr}(W_k^{(m)}H_k).
\tag{8}
$$
...
$$
J_{\mathrm{out}}^{(0)}
=\bar C*(J^{\mathcal D})^{\otimes3}
=\frac18\operatorname{Tr}_{I^3O^3}\bar C.
$$

Determinism of $\bar C$ makes this a valid qubit channel. Its trace also
follows directly from $\operatorname{Tr}\bar C=16$:

$$
\operatorname{Tr}J_{\mathrm{out}}^{(0)}
=\frac18\operatorname{Tr}\bar C=2.
$$

It is independent of $U$, and

$$
\mathbb E_U[J_U]=\frac{I_{PF}}2,
\qquad
\mathbb E_U\frac14\operatorname{Tr}(J_{\mathrm{out}}^{(0)}J_U)
=\frac14.
$$

Because $\Omega^{(0)}$ uses $I_4^{\otimes3}=8(J^{\mathcal D})^{\otimes3}$,

$$
\operatorname{Tr}(\Omega^{(0)}\bar C)=8\times\frac14=2=:A_0.
\tag{9}
$$
...
$$
J_{\mathrm{out}}^{(3)}(U)=\bar C*J_U^{\otimes3}.
$$
...
$$
\operatorname{Tr}(\Omega^{(3)}\bar C)
=\mathbb E_U\frac14\operatorname{Tr}(J_{\mathrm{out}}^{(3)}(U)J_U)
\le1=:A_3.
\tag{10}
$$
...
$$
A_1=\frac{27+8\sqrt2}{18},
\qquad
A_2=\frac{21+8\sqrt2}{18},
\qquad
t_1=\frac1{256},
\qquad
t_2=\frac1{64}.
...
$$
\boxed{
A_m-\operatorname{Tr}(\Omega^{(m)}\bar C)
=t_m\sum_{k=2}^{8}\operatorname{Tr}H_k
+\sum_{k=0}^{8}\operatorname{Tr}(R_k^{(m)}H_k).
}
\tag{12}
$$
...
$$
A_m-\operatorname{Tr}(\Omega^{(m)}\bar C)
\ge t_m\sum_{k=2}^{8}\operatorname{Tr}H_k,
\qquad m=1,2.
\tag{13}
$$
...
\begin{aligned}
F_{\mathrm{ub}}
:={}&\left(\frac\gamma2\right)^3A_0
+3(1-\gamma)\left(\frac\gamma2\right)^2A_1\\
&+3(1-\gamma)^2\left(\frac\gamma2\right)A_2
+(1-\gamma)^3A_3\\
={}&\frac1{24}(2-\gamma)
\left[12+(8\sqrt2-9)\gamma+(3-8\sqrt2)\gamma^2\right].
\end{aligned}
\tag{14}
$$
...
$$
\begin{aligned}
F_{\mathrm{ub}}-\operatorname{Tr}(\Omega_\gamma\bar C)
={}&\left(\frac\gamma2\right)^3
\left[A_0-\operatorname{Tr}(\Omega^{(0)}\bar C)\right]\\
&+3(1-\gamma)\left(\frac\gamma2\right)^2
\left[A_1-\operatorname{Tr}(\Omega^{(1)}\bar C)\right]\\
&+3(1-\gamma)^2\left(\frac\gamma2\right)
\left[A_2-\operatorname{Tr}(\Omega^{(2)}\bar C)\right]\\
&+(1-\gamma)^3
\left[A_3-\operatorname{Tr}(\Omega^{(3)}\bar C)\right].
\end{aligned}
$$
...
\sum_{k=2}^{8}\operatorname{Tr}H_k.
}
\tag{15}
$$
...
$$
\operatorname{Tr}(\Omega_\gamma\bar C)=F_{\mathrm{ub}}.
$$

For $0<\gamma<1$, the coefficient in Eq. (15) is strictly positive. Therefore

$$
\sum_{k=2}^{8}\operatorname{Tr}H_k=0.
$$

Every summand is nonnegative because $H_k\succeq0$. Thus
$\operatorname{Tr}H_k=0$ for every $k=2,\ldots,8$. A positive-semidefinite
